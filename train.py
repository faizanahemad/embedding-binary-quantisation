...
def main():  
    ...
    if "Matryoshka_2bit" in train_modules:
        embedding_model = SentenceTransformerEmbeddingCaller(base_model_name)
        matryoshka_model = MatryoshkaEmbeddingModel(embedding_model, dimension_levels=get_dimension_levels(embedding_dim), train_binary=False, train_two_bit=True, expand_two_bit_to_three_bits=False)
        matryoshka_model.to(device)
        matryoshka_model = train_matryoshka_model(matryoshka_model, dataloader, num_epochs=num_epochs)
    ...
    if "Matryoshka_1bit" in train_modules:
        ...
        embedding_model = SentenceTransformerEmbeddingCaller(base_model_name)
        matryoshka_model = MatryoshkaEmbeddingModel(embedding_model, dimension_levels=get_dimension_levels(embedding_dim), train_binary=True, train_two_bit=False, expand_two_bit_to_three_bits=False)
        matryoshka_model.to(device)
        matryoshka_model = train_matryoshka_model(matryoshka_model, dataloader, num_epochs=num_epochs)
